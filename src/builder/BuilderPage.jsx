...
export default function BuilderPage({ initialView = null }) {
  ...
  function handleDiscordExport() {
    setSidebarOpen(false)
    const text = generateDiscordExport(useBuilderStore.getState())
    navigator.clipboard.writeText(text).then(() => {
      useBuilderStore.getState()._toast('Copied for Discord!')
    }).catch(() => {
      useBuilderStore.getState()._toast('Copy failed — try again')
    })
  }
              ...
              {companyMode === 'campaign' ? (
                <>
                  <div className="sb-action-row">
                    ...
                    <button className="sb-action-btn" onClick={handleDiscordExport}>
                      <SbDiscordIcon />
                      <span>Discord</span>
                    </button>
                  </div>
