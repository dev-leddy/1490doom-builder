...
export default function EndOfGameModal({ onClose }) {
  ...
  const [step, setStep] = useState(1) // 1 = survivors, 2 = captain promo, 3 = win/lose
  ...
  function toggleSurvivor(idx) {
    ...
    // Reset captain promo if survivors change
    setNewCaptainIndex(null)
  }

  function goToStep2() {
    if (needsCaptainPromo) {
      setStep(2)
    } else {
      setStep(3)
    }
        ...
        {step === 3 && (
          <>
            ...
            <div className="eog-win-btns">
              <button
                className={`eog-win-btn ${won === true ? 'selected' : ''}`}
                onClick={() => setWon(true)}
              >Victory</button>
              <button
                className={`eog-win-btn ${won === false ? 'selected' : ''}`}
